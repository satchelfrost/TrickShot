...
ATrickShotCharacter::ATrickShotCharacter()
{
	...
	GunOffset = FVector(300.0f, 0.0f, 10.0f);
...
void ATrickShotCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (ProjectileClass != NULL)
	{
		...
		if (World != NULL)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<ATrickShotProjectile>(ProjectileClass, SpawnLocation, SpawnRotation);
			}
			else
			{
...
void ATrickShotCharacter::OnResetVR()
{
	UHeadMountedDisplayFunctionLibrary::ResetOrientationAndPosition();
}

void ATrickShotCharacter::BeginTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
void ATrickShotCharacter::EndTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
//void ATrickShotCharacter::TouchUpdate(const ETouchIndex::Type FingerIndex, const FVector Location)
//{
...
void ATrickShotCharacter::MoveForward(float Value)
{
	if (Value != 0.0f)
	{
...
void ATrickShotCharacter::MoveRight(float Value)
{
	if (Value != 0.0f)
	{
		// add movement in that direction
...
void ATrickShotCharacter::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void ATrickShotCharacter::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerPitchInput(Rate * BaseLookUpRate * GetWorld()->GetDeltaSeconds());
}

bool ATrickShotCharacter::EnableTouchscreenMovement(class UInputComponent* PlayerInputComponent)
{
	if (FPlatformMisc::SupportsTouchInput() || GetDefault<UInputSettings>()->bUseMouseForTouch)
	{
		PlayerInputComponent->BindTouch(EInputEvent::IE_Pressed, this, &ATrickShotCharacter::BeginTouch);
		PlayerInputComponent->BindTouch(EInputEvent::IE_Released, this, &ATrickShotCharacter::EndTouch);

		//Commenting this out to be more consistent with FPS BP template.
		//PlayerInputComponent->BindTouch(EInputEvent::IE_Repeat, this, &ATrickShotCharacter::TouchUpdate);
		return true;
	}
